...
class DeployToDo extends Component
{
    ...
    private function checkForAppDeployment(){
        if(isset( $this->deploy['workspace'] ) && isset( $this->deploy['workspace']['app_id'] )){
            $this->app_id = $this->deploy['workspace']['app_id'];
            $this->api_key = $this->deploy['workspace']['api_key'];
    ...
    private function writeToDeployFile($id, $key, $deployFileArray){
        $deployFileArray['workspace']['app_id'] = $id;
        $deployFileArray['workspace']['api_key'] = $key;
    ...
    public function deploy(){
        ...
        if(!isset($this->app_id)){
            ...
            $finalJSONPayload = str_replace('${workspace.name}', str_replace('_', '-', $repoName), $finalJSONPayload);
            //dd($this->repo);
            $finalJSONPayload = str_replace('${workspace.repo}', $this->repo, $finalJSONPayload);
    ...
    {
        return view('livewire.workspace.deploy-to-do');
    }
}
