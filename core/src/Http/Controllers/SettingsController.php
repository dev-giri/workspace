...
use Workspace\ApiKey;
use Workspace\KeyValue;

class SettingsController extends Controller
{
    public function index($section = ''){
        if(empty($section)){
            return redirect(route('workspace.settings', 'profile'));
        }
    ...
    public function profilePut(Request $request){
        ...
        foreach(config('workspace.profile_fields') as $key){
            if(isset($request->{$key})){
                $type = $key . '_type__workspace_keyvalue';
    ...
    public function securityPut(Request $request){
        ...
        $validator = Validator::make($request->all(), [
            ...
            'password' => 'required|confirmed|min:'.config('workspace.auth.min_password_length'),
        ]);
    ...
    public function invoice(Request $request, $invoiceId) {
        return $request->user()->downloadInvoice($invoiceId, [
            'vendor'  => setting('site.title', 'Workspace'),
