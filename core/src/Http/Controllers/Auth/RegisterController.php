...
namespace Workspace\Http\Controllers\Auth;
...
use Workspace\Notifications\VerifyEmail;
...
class RegisterController extends Controller
{
    ...
    */
    ...
    public function complete(Request $request){
        ...
        return redirect()->route('workspace.dashboard')->with(['message' => 'Successfully updated your profile information.', 'message_type' => 'success']);
    ...
    public function showRegistrationForm()
    {
        if(setting('billing.card_upfront')){
            return redirect()->route('workspace.pricing');
        }
    ...
    public function getUniqueUsernameFromEmail($email)
    {
        ...
        $user_exists = \Workspace\User::where('username', '=', $username)->first();
        $counter = 1;
        while (isset($user_exists->id) ) {
            $new_username = $username . $counter;
            $counter += 1;
            $user_exists = \Workspace\User::where('username', '=', $new_username)->first();
        }
