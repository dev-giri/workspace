...
use Workspace\PaddleSubscription;
use Carbon\Carbon;
use Workspace\Plan;
use Workspace\User;
...
class SubscriptionController extends Controller
{
    ...
    public function __construct(){
        $this->vendor_auth_code = config('workspace.paddle.auth_code');
        $this->vendor_id = config('workspace.paddle.vendor');

        $this->paddle_checkout_url = (config('workspace.paddle.env') == 'sandbox') ? 'https://sandbox-checkout.paddle.com/api' : 'https://checkout.paddle.com/api';
        $this->paddle_vendors_url = (config('workspace.paddle.env') == 'sandbox') ? 'https://sandbox-vendors.paddle.com/api' : 'https://vendors.paddle.com/api';
    }
                ...
                if($order->is_subscription && $plans->contains('plan_id', $order->product_id) ){
                    ...
                    if(auth()->guest()){
                        ...
                        if(User::where('email', $subscription->user_email)->exists()){
                            ...
                            // create a new user
                            $registration = new \Workspace\Http\Controllers\Auth\RegisterController;
