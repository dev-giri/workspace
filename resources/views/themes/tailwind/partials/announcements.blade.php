...
<div id="announcement" class="fixed bottom-0 right-0 z-50 flex items-end justify-center px-4 py-6 transition-all duration-200 ease-out transform translate-y-0 opacity-100 cursor-pointer pointer-events-none hover:-translate-y-1 sm:p-6 sm:items-start sm:justify-end" data-href="{{ route('workspace.announcement', $announcement->id) }}">
	<div id="announcement_content" class="relative w-full max-w-sm bg-white border border-gray-200 rounded-lg shadow-lg pointer-events-auto">
		<div class="relative p-5">
			...
			<div id="announcement_footer" class="mt-1"><a href="{{ route('workspace.announcement', $announcement->id) }}" class="text-sm font-medium leading-5 text-indigo-600 transition duration-150 ease-in-out hover:text-indigo-500 focus:outline-none focus:underline">Learn More</a></div>
		</div>
	</div>
</div>
...
<script>
	...
	function markNotificationsRead(endpoint, splitPopReadyState){
		...
		HttpRequest.open("POST", "{{ route('workspace.announcements.read') }}", true);
