<div class="p-8">
    ...
    @else
        ...
        @else
            <p class="text-gray-600">Please <a href="{{ route('workspace.settings', 'plans') }}">Subscribe to a Plan</a> in order to see your subscription information.</p>
            <a href="{{ route('workspace.settings', 'plans') }}" class="inline-flex self-start justify-center w-auto px-4 py-2 mt-5 text-sm font-medium text-white transition duration-150 ease-in-out border border-transparent rounded-md bg-workspace-600 hover:bg-workspace-500 focus:outline-none focus:border-workspace-700 focus:shadow-outline-workspace active:bg-workspace-700">View Plans</a>
        @endif
    @endif
</div>
<script>
