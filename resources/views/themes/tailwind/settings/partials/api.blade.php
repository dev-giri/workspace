<div class="flex flex-col px-10 py-8">
	<form action="{{ route('workspace.settings.api.post') }}" method="POST">
		<div>
		...
		<div class="flex justify-end w-full mt-2">
			<button class="flex self-end justify-center w-auto px-4 py-2 mt-5 text-sm font-medium text-white transition duration-150 ease-in-out border border-transparent rounded-md bg-workspace-600 hover:bg-workspace-500 focus:outline-none focus:border-workspace-700 focus:shadow-outline-workspace active:bg-workspace-700" dusk="update-profile-button">Create New Key</button>
		</div>
			...
			<div x-data="{ open: false, id: '', key: '', name: '' }" x-init="
				...
				<div class="flex items-end justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
					...
					<div x-show="$store.editApiKey.open" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" class="inline-block px-4 pt-5 pb-4 overflow-hidden text-left align-bottom transition-all transform bg-white rounded-lg shadow-xl sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6" role="dialog" aria-modal="true" aria-labelledby="modal-headline">
						...
						<div class="flex flex-col justify-between w-full mt-2">
							<form action="{{ route('workspace.settings.api.put') }}" method="POST">
								<div>
								...
								<p class="flex justify-end mt-5">
									...
									<button type="submit" class="inline-flex justify-center px-4 py-2 text-base font-medium leading-6 text-white transition duration-150 ease-in-out border border-transparent rounded-md shadow-sm bg-workspace-600 hover:bg-workspace-500 focus:outline-none focus:border-workspace-700 focus:shadow-outline-workspace sm:text-sm sm:leading-5">Update</button>
								</p>
								{{ csrf_field() }}
							</form>
						</div>
						...
						<div class="flex flex-col justify-between w-full mt-2">
							...
							<div class="mt-5 sm:mt-6 sm:flex sm:flex-row-reverse">
							    <span class="flex w-full rounded-md shadow-sm sm:ml-3 sm:w-auto">
									<form action="{{ route('workspace.settings.api.delete') }}" method="POST">
