...
@section('content')
    ...
    <div class="sm:mx-auto sm:w-full sm:max-w-md sm:pt-10">
        ...
        <p class="mt-4 text-sm leading-5 text-center text-gray-600 max-w">
            or, you can
            <a href="{{ route('login') }}" class="font-medium transition duration-150 ease-in-out text-workspace-600 hover:text-workspace-500 focus:outline-none focus:underline">
                login here
            </a>
        </p>
    </div>
    ...
    <div class="flex flex-col justify-center pb-10 sm:pb-20 sm:px-6 lg:px-8">
        ...
        <div class="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
            <div class="px-4 py-8 bg-white border shadow border-gray-50 sm:rounded-lg sm:px-10">
                <form role="form" method="POST" action="@if(setting('billing.card_upfront')){{ route('workspace.register-subscribe') }}@else{{ route('register') }}@endif">
                    @csrf
                    ...
                    <div class="flex flex-col items-center justify-center text-sm leading-5">
                        <span class="block w-full mt-5 rounded-md shadow-sm">
                            <button type="submit" class="flex justify-center w-full px-4 py-2 text-sm font-medium text-white transition duration-150 ease-in-out border border-transparent rounded-md bg-workspace-600 hover:bg-workspace-500 focus:outline-none focus:border-workspace-700 focus:shadow-outline-workspace active:bg-workspace-700">
                                Register
                            </button>
                        </span>
                        <a href="{{ route('login') }}" class="mt-3 font-medium transition duration-150 ease-in-out text-workspace-600 hover:text-workspace-500 focus:outline-none focus:underline">
