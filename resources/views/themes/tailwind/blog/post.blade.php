...
@section('content')
    ...
    <div class="max-w-4xl px-5 mx-auto mt-10 lg:px-0">
        <a href="{{ route('workspace.blog') }}" class="flex items-center mb-6 font-mono text-sm font-bold cursor-pointer text-workspace-500">
    ...
    <article id="post-{{ $post->id }}" class="max-w-4xl px-5 mx-auto prose prose-xl lg:prose-2xl lg:px-0">
        ...
        <div class="max-w-4xl mx-auto mt-6">
            ...
            <h1 class="flex flex-col leading-none">
                ...
                <span class="mt-0 mt-10 text-base font-normal">Written on <time datetime="{{ Carbon\Carbon::parse($post->created_at)->toIso8601String() }}">{{ Carbon\Carbon::parse($post->created_at)->toFormattedDateString() }}</time>. Posted in <a href="{{ route('workspace.blog.category', $post->category->slug) }}" rel="category">{{ $post->category->name }}</a>.</span>
            </h1>
